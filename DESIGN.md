...
| 2026-05-21 | § 8.5 Checkout / § 8.7 Account / § 6 nouveau | **Autocomplétion d'adresse BAN** branchée sur Checkout + AccountPage. Nouveau composant réutilisable `src/components/AddressAutocomplete.jsx` : pattern combobox ARIA complet (`role="combobox"` + `aria-expanded` + `aria-controls` + `aria-activedescendant`), navigation clavier (Arrow Up/Down, Enter, Escape), debounce 250 ms, gestion erreurs + état loading + état vide. Wrapper API `src/api/geoApi.js` consomme `/api/geo/address` (proxy BAN côté back). Sélection d'une suggestion remplit auto adresse1 + code postal + ville + région + pays. L'utilisateur peut toujours éditer manuellement. Clés i18n `common.addressAutocomplete` traduites natives FR/EN/AR/HE | Sprint 2.5f |
| 2026-05-21 | § 0 PWA | **Application installable** : `public/manifest.webmanifest` (theme `#0B0B23`, icônes 192/512, scope `/`, mode `standalone`) + `public/sw.js` (service worker minimaliste sans Workbox, pas de dépendance ajoutée). Stratégies de cache : (a) shell precache au `install`, (b) assets statiques en `stale-while-revalidate`, (c) navigations en `network-first` avec fallback shell, (d) calls `/api/*` jamais mis en cache (données toujours fraîches). Enregistré dans `main.jsx` uniquement en `import.meta.env.PROD` pour ne pas casser le HMR Vite en dev. Meta `theme-color`, `apple-touch-icon` et `apple-mobile-web-app-*` ajoutés à `index.html` pour iOS Safari | Sprint 2.5f PWA |
