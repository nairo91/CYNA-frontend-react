...
import { AlertCircle, CheckCircle2, KeyRound, LogIn, X } from 'lucide-react'
...
export function LoginPage() {
  ...
  const handleGoogleConnect = () => {
    ...
    window.location.assign(`${API_BASE_URL}/login/google`)
  }
  ...
  return (
    ...
    >
      <button
        ...
        disabled={isConnecting || Boolean(twoFactorChallenge)}
        className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-border bg-background text-sm font-medium text-foreground transition-colors hover:bg-accent disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card"
      >
        {isConnecting ? (
        ...
        {isConnecting ? 'Connexion Google...' : page.google}
      </button>
      ...
      <form onSubmit={handleSubmit} className="grid gap-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <label className="inline-flex items-center gap-2 text-sm text-foreground">
            <input
              ...
              onChange={handleChange}
              disabled={Boolean(twoFactorChallenge)}
              className="h-4 w-4 rounded border-border accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-60"
            />
            {page.rememberMe}
          </label>
          <Link
        ...
        {errorMessage && !twoFactorChallenge ? (
          <div
            role="alert"
        ...
        <button
          type="submit"
          disabled={isSubmitting || Boolean(twoFactorChallenge)}
          ...
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-2xl">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 id="two-factor-title" className="text-lg font-semibold text-foreground">
              ...
              <button
                ...
                aria-label="Annuler la validation du code"
              >
            ...
            <form onSubmit={handleTwoFactorSubmit} className="grid gap-4">
              ...
              >
                ...
                {isSubmitting ? page.submitting : page.twoFactorSubmit}
              </button>
            </form>
          </div>
        </div>
